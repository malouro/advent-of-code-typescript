...
async function runPuzzle(year: number, day: number, part?: number, inputFile?: string) {
  ...
  if (!targetInputFile) {
    if (!structure.inputFile) {
      throw new Error(
        `No input file found for year ${year}, day ${day}. Available files: star.txt, test.txt, sample1.txt, sample.txt`,
      );
    }
  ...
  if (structure.hasSolver) {
    ...
    throw new Error(
      `No solver found for year ${year}, day ${day}. Expected solver.js or partOne.js/partTwo.js`,
    );
  }
}
...
const parser = yargs(hideBin(process.argv))
  ...
  .command('list', 'List all available puzzles', {}, async () => {
      ...
      for (const year of years) {
        ...
        for (const day of days) {
          ...
          try {
            ...
            const inputInfo = structure.inputFile
              ? `(${structure.inputFile.split('/').pop()})`
              : '(no input)';
